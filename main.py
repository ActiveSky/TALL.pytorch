...
def train(epoch: int, model: TALL, trainloader: DataLoader, optimizer):
    model.train()
    train_loss = 0
...
def test(epoch: int, model: TALL, test_dataset: TestDataset, test_result_output, path: str):
    global best_R1_IOU5
    global best_R5_IOU5
...
def main(config:Namespace):
    ...
    train_dataset = TrainingDataSet(
        sliding_clip_path='exp_data/Interval64_128_256_512_overlap0.8_c3d_fc6/',
        clip_sentence_vector_path='exp_data/TACoS/train_clip-sentvec.pkl')
    # 2. load test dataset
    test_dataset = TestDataset(
        sliding_clip_path='exp_data/Interval128_256_overlap0.8_c3d_fc6/',
        clip_sentence_vector_path='exp_data/TACoS/test_clip-sentvec.pkl')
    
    # 3. build dataloader for train and test dataset
    ...
    setup_seed(global_seed)  # default seed is 0
